...
def pickItem(celldict, lenlimit=2):
    for indx in celldict.keys():
        if len(celldict[indx]) == lenlimit:
            return (indx, celldict[indx])
    return pickItem(celldict, takenlst, lenlimit + 1)


def tryNum(datastr, ilst, celldict):
    count = 0
    while datastr.count('0') > 0:
        new_celldict = celldict if not new_celldict else new_celldict
        indx, set_ = pickItem(celldict, takenlst)
        temp = datastr
        for num in set_:
            temp[indx] = num
            new_celldict = getCellSetDict(temp, ilst)
            if new_celldict:
                changes = makeDataChanges(datastr, new_celldict)
                if changes:
                    datastr = changes
        count += 1
        if count%50==0:
...
def processData(datastr):
    ...
    while datastr.count('0') > 0:
        ...
        if not changes:
            datastr = tryNum(datastr, ilst, celldict)
        else:
            datastr = changes
        print_data(datastr)
        print('*'*15)
...
data_str = file_inpt("tst2.py", 'r', encoding='utf8')
print_data(data_str)
print('*'*15)
