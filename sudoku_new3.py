...
def getCellSetDict(datastr, ilst, forchanges=None):
    ...
    edited_celldict = applyAllNakedSets(getAllArraysSetsDict(all_arrays_dict, celldict), celldict)
...
def runTestsBck(datastr, ilst, prev_sets={}, prev_picks=[], prev_data=[], inicialized=None):
    ...
    if temp_data.count('0') == 0:
        if not checkLegal(temp_data, ilst):
            ...
            return runTestsBck(temp_data, ilst, prev_sets, prev_picks, prev_data)
        return temp_data
    else:
        if not inicialized:
            while True:
                new_legit_celldict = getCellSetDict(temp_data, ilst)
                if not new_legit_celldict or not checkLegal(temp_data, ilst):
                    prev_data = prev_data[:-1]
                    temp_data = prev_data[-1]
                    return runTestsBck(temp_data, ilst, prev_sets, prev_picks, prev_data)
        ...
        #print(picknum)
        return runTestsBck(temp_data, ilst, prev_sets, prev_picks+[(indx, picknum)], prev_data+[temp_data])   
'''
...
print_data(data_str2)
print('*'*15)
processData(list(data_str2))
